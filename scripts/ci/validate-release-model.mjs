...
if (isTemplate) {
  ...
  check(!candidate.includes('BEPLY_PLUGIN_TEMPLATE_READ_TOKEN'), 'Public template checkout must not require a consumer secret')
  check(!promotion.includes('BEPLY_PLUGIN_TEMPLATE_READ_TOKEN'), 'Public template checkout must not require a consumer secret')
